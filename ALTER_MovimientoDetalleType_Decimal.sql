/*
	Corrige un desajuste real de tipos que quedo expuesto al probar el Paso 3:
	MovimientoDetalleType.Cantidad esta declarado como int, pero MovimientosDetalle.Cantidad
	(la tabla real) y ProductosStock.Cantidad son decimal(18,2). Como ya se definio que
	Cantidad Unidad puede ser decimal (para permitir fracciones de caja, ej. 0.5 caja con
	Conversion=12 -> 6 unidades), pasar por un TVP tipado como int redondeaba el valor en
	silencio, sin error, justo antes de llegar al SP.

	Que hace este script:
	1. Quita [dbo].[MAM_Movimientos] (un procedimiento no se puede ALTER para dejar de usar un
	   tipo; hay que sacarlo de en medio para poder borrar el tipo).
	2. Borra y vuelve a crear [dbo].[MovimientoDetalleType], con Cantidad decimal(18,2) en vez
	   de int. Las demas columnas quedan exactamente iguales.
	3. Vuelve a crear [dbo].[MAM_Movimientos] con el mismo cuerpo que ya esta corriendo hoy
	   (incluye los fixes de Conversion/UnidadMaxima del Paso 1, precio/Total del Paso 3, y las
	   correcciones de GROUP BY / SUM con subconsulta de la vuelta anterior) - no cambia nada de
	   esa logica, solo re-declara el procedimiento porque hubo que borrarlo.

	Todo el script corre dentro de una sola transaccion con SET XACT_ABORT ON: si cualquier
	paso falla (por ejemplo, algun otro objeto todavia depende de MovimientoDetalleType), se
	revierte todo y quedas exactamente como estabas antes de correrlo, sin el procedimiento a
	medio borrar.

	IMPORTANTE: te toca revisar del lado VB.NET que la parte que arma el TVP (
	_fnProyectarDetalleMovimiento en AccesoLogica.vb) tambien pase a usar Decimal para
	Cantidad en vez de Integer - ya te mando ese archivo actualizado por separado, para que
	viajen juntos.
...
DROP PROCEDURE [dbo].[MAM_Movimientos]
GO

DROP TYPE [dbo].[MovimientoDetalleType]
GO

CREATE TYPE [dbo].[MovimientoDetalleType] AS TABLE(
	[Id] [int] NULL,
...
CREATE PROCEDURE [dbo].[MAM_Movimientos](@tipo int, @id int=-1, @ConceptoId int=-1,@TransaccionId int=-1,@DepositoId int=-1,
