/*
	PASO 1 de la funcionalidad "Cantidad Unitaria / Cantidad Maxima (caja)" en Movimientos.

	Que cambia en este script, y por que:
	- @tipo=7 (lista de productos para elegir en el movimiento) y @tipo=4 (detalle de un
	  movimiento ya guardado) ahora tambien traen: CodigoBarras, UnidadVentaId, UnidadMaximaId,
	  el nombre de cada unidad (via ClasificadorDetalle, mismo patron que ya usan con "industria"),
	  y Conversion.
	- Conversion sale con ISNULL(NULLIF(Conversion,0),1): si el producto no tiene Unidad Maxima
	  configurada (Conversion en NULL o en 0), el VB va a recibir 1 -> 1 caja = 1 unidad, tal
	  cual lo pediste.
	...
	produccion, y confirmarme que el SP recompila sin error antes de que yo siga con el
	paso 2 (los formularios VB.NET que van a usar estas columnas nuevas).
*/
