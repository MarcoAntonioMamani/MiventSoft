...
Public Class CrearCategoria
    Public IdCategoria As Integer = -1
    ...
    Private Sub ButtonX1_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        If (tbdescripcion.Text.Length > 1) Then
            Dim id As String = ""
            Bandera = L_prCategoriaInsertar(id, tbdescripcion.Text, tbdescripcion.Text, 1, "", 1, 1)
            If (Bandera = True) Then

                IdCategoria = CInt(id.Trim)
                Me.Close()
            Else
                ToastNotification.Show(Me, "Error Al guardar La Categoria", img, 8000, eToastGlowColor.Red, eToastPosition.TopCenter)
                tbdescripcion.Focus()
            End If
