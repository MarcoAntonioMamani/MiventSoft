...
Public Class Tec_VentasTigoMoney
#Region "Atributos"
...
#Region "METODOS PRIVADOS"
    ...
    Private Sub _prCargarDetalleVenta(_numi As String)
        ...
        dt = ListaVentasDetallesServicios(_numi)
...
salirIf:
    ...
    Private Sub grdetalle_CellValueChanged(sender As Object, e As ColumnActionEventArgs) Handles grDetalle.CellValueChanged
                    ...
                    If (TipoProgramas = 1) Then
                        ...
                        Else
                            ...
                            If (TipoProgramas = 1) Then
                                ...
                                CType(grDetalle.DataSource, DataTable).Rows(pos).Item("PorcentajeGanado") = 0
                                CType(grDetalle.DataSource, DataTable).Rows(pos).Item("CantidadGanado") = 0
                                ...
                                grDetalle.SetValue("Cantidad", 1)
                                grDetalle.SetValue("ProcentajeDescuento", 0)
                                grDetalle.SetValue("PorcentajeGanado", 0)
                                grDetalle.SetValue("CantidadGanado", 0)
                                grDetalle.SetValue("MontoDescuento", 0)
                                grDetalle.SetValue("SubTotal", grDetalle.GetValue("Precio"))
                                grDetalle.SetValue("Total", grDetalle.GetValue("Precio"))
        ...
        If (e.Column.Index = grDetalle.RootTable.Columns("PorcentajeGanado").Index) Then
            If (Not IsNumeric(grDetalle.GetValue("PorcentajeGanado")) Or grDetalle.GetValue("PorcentajeGanado").ToString = String.Empty) Then
                ...
                CType(grDetalle.DataSource, DataTable).Rows(pos).Item("PorcentajeGanado") = 0
                CType(grDetalle.DataSource, DataTable).Rows(pos).Item("CantidadGanado") = 0
                CType(grDetalle.DataSource, DataTable).Rows(pos).Item("Cantidad") = CType(grDetalle.DataSource, DataTable).Rows(pos).Item("Cantidad")
                ...
                'grdetalle.SetValue("SubTotal", grdetalle.GetValue("tbpbas"))
            Else
                If (grDetalle.GetValue("PorcentajeGanado") > 0 And grDetalle.GetValue("PorcentajeGanado") <= 100) Then

                    Dim porcdesc As Double = grDetalle.GetValue("PorcentajeGanado")
                    Dim montodesc As Double = (grDetalle.GetValue("Cantidad") * (porcdesc / 100))
                    lin = grDetalle.GetValue("Id")
                    pos = -1
                    _fnObtenerFilaDetalle(pos, lin, grDetalle.GetValue("Tipo"))
                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("CantidadGanado") = montodesc
                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("Cantidad") = montodesc + grDetalle.GetValue("Cantidad")
                    grDetalle.SetValue("CantidadGanado", montodesc)
                    grDetalle.SetValue("Cantidad", montodesc + grDetalle.GetValue("Cantidad"))
                ...
                Else
                    ...
                    grDetalle.SetValue("PorcentajeGanado", 0)
                    grDetalle.SetValue("CantidadGanado", 0)
                    grDetalle.SetValue("cantidad", grDetalle.GetValue("Cantidad"))
        ...
        '''''''''''''''''''''MONTO DE INCREMENTO '''''''''''''''''''''
        If (e.Column.Index = grDetalle.RootTable.Columns("CantidadGanado").Index) Then
            If (Not IsNumeric(grDetalle.GetValue("CantidadGanado")) Or grDetalle.GetValue("CantidadGanado").ToString = String.Empty) Then
                ...
                'grdetalle.SetValue("SubTotal", grdetalle.GetValue("tbpbas"))
            Else
                If (grDetalle.GetValue("CantidadGanado") > 0 And grDetalle.GetValue("CantidadGanado") <= grDetalle.GetValue("CantidadGanado")) Then

                    Dim montodesc As Double = grDetalle.GetValue("CantidadGanado")
                    Dim pordesc As Double = ((montodesc * 100) / grDetalle.GetValue("Cantidad"))

                    lin = grDetalle.GetValue("Id")
                    pos = -1
                    _fnObtenerFilaDetalle(pos, lin, grDetalle.GetValue("Tipo"))
                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("CantidadGanado") = montodesc
                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("PorcentajeGanado") = pordesc

                    grDetalle.SetValue("PorcentajeGanado", pordesc)
                    grDetalle.SetValue("cantidad", montodesc + grDetalle.GetValue("Cantidad"))
...
#Region "METODOS SOBRECARGADOS"
        ...
        Try
            ...
            If (swTipoVenta.Value = True) Then
                ...
                If (bandera = True) Then
                    ...
                    Dim dt As DataTable = ListaVentasServiciosDetallePago(-1)
    ...
    Public Sub _PMOEliminarRegistro()
        ...
        If (bandera = True) Then
            ...
            Dim res As Boolean
            Try
                res = L_prBorrarRegistro(tbCodigo.Text, mensajeError, "MAM_VentasServicios")
                If res Then
