...
Public Class Tec_VentasDetalle
    ...
    Public Sub InsertarProductosSinLote(cantidad As Double, Precio As Double)
        'a.Id , a.VentaId, a.ProductoId, p.NombreProducto As Producto, a.Cantidad, a.Precio, a.SubTotal,
        'a.ProcentajeDescuento, a.MontoDescuento, a.Total, a.Detalle, a.PrecioCosto, a.Lote, a.FechaVencimiento,
        '1 As estado, cast('' as image ) as img
        ', 0 as stock
        ...
        If ((Not existe)) Then
            ...
            If (grProducto.GetValue("Tipo") = 1) Then  ''Inserto Productos
                ...
                If ((pos >= 0)) Then
                    ...
                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("PrecioCaja") = Precio

                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("Precio") = Precio / grProducto.GetValue("QTY")


                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("SubTotal") = (Precio / grProducto.GetValue("QTY")) * cantidad

                    CType(grDetalle.DataSource, DataTable).Rows(pos).Item("Total") = (Precio / grProducto.GetValue("QTY")) * cantidad
    ...
    Public Sub seleccionarProducto()
        If (IsNothing(FilaSelectLote)) Then
            ...
            Else
                ...
                ef.ProductoId = grProducto.GetValue("id")
                If (TipoProgramas = 1) Then
                    ef.TipoMovimiento = 3
                Else
                    ef.TipoMovimiento = 4
                ...
                If (bandera = True) Then
                    InsertarProductosSinLote(ef.CantidadTransaccion, ef.Precio)
