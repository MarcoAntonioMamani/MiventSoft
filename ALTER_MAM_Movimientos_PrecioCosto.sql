/*
	PASO 1 (ya aplicado) + FIX de "precio" que aparecio al probar el Paso 3.

	FIX de este script (lo nuevo):
	- El VB (Tec_Movimientos.vb / Tec_MovimientoDetalle.vb) configura columnas "precio" y
	  "Total" en la grilla de detalle, pero @tipo=4 (detalle de un movimiento ya guardado)
	  y @tipo=7 (lista de productos para elegir) nunca las devolvian -> "no encuentra el
	  precio" al abrir el detalle. MovimientosDetalle no guarda precio (a diferencia de
	  CompraDetalle, que si tiene PrecioCosto); el precio siempre se lee en vivo desde
	  Precios, igual que ya hacen otras partes de este mismo SP y de MAM_Compras.
	- Se agrega "precio" (subconsulta a Precios, PrecioCategoriaId = categoria Tipo=0 "Costo
	  Compra", resuelta con la misma logica que ya usa MAM_Compras: select Min(Id) from
	  PreciosCategorias where Tipo=0) a @tipo=4 y @tipo=7. En @tipo=4 tambien se agrega
	  "Total" = Cantidad * precio.
	- Mismo problema en @tipo=3 (buscador/listado de movimientos, la grilla de arriba):
	  _PMOGetListEstructuraBuscador() en Tec_Movimientos.vb ya esperaba una columna
	  "TotalMovimiento" que el SP tampoco devolvia. Se agrega como la suma, por movimiento,
	  de Cantidad*precio de todo su detalle (misma resolucion de precio que en @tipo=4/7).
	- AlmacenId se filtra en 1, igual que ya hacen los otros dos lugares de este sistema que
	  leen Precios sin pasar el deposito real (@tipo=18 de este mismo SP, y el reporte de
	  Saldos Valorados). OJO: @DepositoId no llega hoy a @tipo=4 (L_prListarDetalleMovimiento
	  no lo manda), asi que no se puede filtrar Precios por el deposito real del movimiento
	  sin tocar tambien AccesoLogica.vb y el VB que lo llama - eso queda fuera de este fix,
	  avisame si tu negocio realmente maneja precios distintos por deposito.
	- Si el producto no tiene una fila en Precios para esa categoria, precio sale en 0
	  (ISNULL), no se cae la fila del detalle (por eso subconsulta con ISNULL, no INNER JOIN).

	PASO 1 (lo que ya estaba, sin cambios):
	- @tipo=7 y @tipo=4 ya traian: CodigoBarras, UnidadVentaId, UnidadMaximaId, el nombre de
	  cada unidad (via ClasificadorDetalle, mismo patron que ya usan con "industria"), y
	  Conversion (con ISNULL(NULLIF(Conversion,0),1): sin Unidad Maxima configurada, 1 caja =
	  1 unidad).
	- No se toca ninguna tabla, no se agrega nada a MovimientosDetalle: la cantidad que se
	  graba sigue siendo unicamente la cantidad en unidades, como ya funciona hoy.

	Recomendacion: correr esto primero en un ambiente de pruebas/desarrollo, no directo en
	produccion.
*/
...
--drop procedure MAM_Movimientos
