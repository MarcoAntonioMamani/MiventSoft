/* ============================================================================
   MAM_Ventas - v2: registra el incremento diario APLICADO a cada venta
   Base de datos: DistribucionDistralKCP2023
   Generado con Claude para Marco Mamani (BANCOSOL)
   Base: MAM_Ventas_PARCHEADO.sql (el parche de auditoria ya aplicado)

   QUE SE AGREGA en esta version (todo aditivo, no se toca nada de lo de
   arriba salvo lo listado aca):

   1) Tabla NUEVA dbo.VentasIncrementoAplicado (Id, VentaId, Monto,
      FechaHoraRegistro): 1 fila por venta NUEVA creada desde el Escritorio,
      con el monto de incremento que efectivamente se le aplico ese dia.
      Es la fuente de verdad histórica: de aca en mas, CUALQUIER calculo de
      total (recibo, reportes futuros) debe leer este valor en vez de
      asumir "el incremento de hoy". No se toca la tabla Ventas (evita tocar
      los 3 INSERT INTO Ventas VALUES(...) posicionales que existen entre
      MAM_Ventas y sp_go_TC004_appMovil - ver nota de riesgo mas abajo).

   2) Parametro nuevo @IncrementoVenta decimal(18,2)=0.

   3) @tipo=1 (Nuevo Registro - Escritorio): despues del INSERT INTO Ventas
      de siempre (sin tocarlo), inserta 1 fila en VentasIncrementoAplicado
      con el monto recibido en @IncrementoVenta.

   4) @tipo=6 (Recibo de Venta): se agrega un LEFT JOIN a
      VentasIncrementoAplicado y la columna IncrementoVenta al SELECT, para
      que el recibo impreso pueda sumarla al total en vez de recalcular
      sin ella.

   5) @tipo=3 (Mostrar Ventas - el buscador que usa Tec_Ventas): mismo LEFT
      JOIN y columna IncrementoVenta. Asi, cuando Marco navega/ve una venta
      ya guardada, el campo "Incremento Adicional" y el Total Venta en
      pantalla muestran el monto REALMENTE registrado para ESA venta (0 si
      es una venta vieja de antes de este feature, o una venta movil - que
      todavia no aplican el incremento), en vez de mostrar siempre el monto
      configurado para HOY sin importar la venta que se este viendo.

   NO TOCADO: sp_go_TC004_appMovil (ventas por App Movil) - a proposito,
   porque el incremento diario todavia NO aplica a las ventas moviles (eso
   quedo pendiente como tema aparte). Las ventas moviles simplemente no
   van a tener fila en VentasIncrementoAplicado, y el LEFT JOIN + ISNULL
   en @tipo=6 ya contempla eso (les suma 0).

   RIESGO IMPORTANTE que motivo este diseño (tabla nueva en vez de columna
   en Ventas): Ventas se llena con "INSERT INTO Ventas VALUES(...)" SIN
   lista de columnas, en 3 lugares (MAM_Ventas @tipo=1, y sp_go_TC004_appMovil
   dos veces). Agregar una columna a Ventas con ALTER TABLE rompe los 3 en
   el momento en que se ejecuten, porque cambia la cantidad de columnas
   esperadas - incluye las 2 del movil, que quedarian rotas AUNQUE no uses
   el incremento ahi. Por eso se opto por una tabla nueva, referenciada por
   VentaId, que no toca el esquema de Ventas para nada.

   RECOMENDACION: probar en un ambiente de desarrollo/pruebas antes de
   correr contra produccion.
   ============================================================================ */

IF OBJECT_ID('dbo.VentasIncrementoAplicado', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.VentasIncrementoAplicado (
        Id                 INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        VentaId            INT NOT NULL,
        Monto              DECIMAL(18,2) NOT NULL,
        FechaHoraRegistro  DATETIME2(3) NOT NULL,
        CONSTRAINT UQ_VentasIncrementoAplicado_VentaId UNIQUE (VentaId),
        CONSTRAINT FK_VentasIncrementoAplicado_Ventas FOREIGN KEY (VentaId) REFERENCES dbo.Ventas(Id)
    );
END
GO

/* ============================================================================
   A PARTIR DE ACA: el mismo parche de auditoria que ya tenias (sin cambios,
   salvo los 3 puntos marcados con --==== INCREMENTO (INICIO/FIN) ==== )
   ============================================================================ */
...
,@IncrementoVenta decimal(18,2)=0)
AS
BEGIN
	...
	BEGIN
        ...
        'TIPO 1 - Registrar Venta';
		  set @Id=IIF((select COUNT(Id) from Ventas)=0,0,(select MAX(Id) from Ventas))+1
			...
			--==== INCREMENTO (INICIO) - registrar el monto de incremento diario aplicado a ESTA venta ====
			--    Aislado en su propio TRY/CATCH: si por lo que sea falla, NO afecta el registro de la venta real.
			BEGIN TRY
				INSERT INTO VentasIncrementoAplicado (VentaId, Monto, FechaHoraRegistro)
				VALUES (@Id, ISNULL(@IncrementoVenta, 0), SYSDATETIME())
			END TRY
			BEGIN CATCH
				INSERT INTO Bitacora (banum,baproc,balinea,bamensaje,batipo,bafact,bahact,bauact)
					   VALUES(ERROR_NUMBER(),ERROR_PROCEDURE(),ERROR_LINE(),
					   CONCAT('INCREMENTO TIPO 1 | VentaId: ',@Id,' | Usuario: ',@usuario,' | Error: ',ERROR_MESSAGE()),
					   1,@newFecha,@newHora,@usuario)
			END CATCH
			--==== INCREMENTO (FIN) ====
    ...
    ROUND(
    ...
    ) AS DistanciaKm, ISNULL(inc.Monto, 0) AS IncrementoVenta
		from Ventas as a
		...
		left join dbo.VentasIncrementoAplicado as inc on inc.VentaId = a.Id
		...
		, Sum(detalle.Precio) as Precio, Sum(detalle.SubTotal) as SubTotal, 
                  Sum(detalle.MontoDescuento) as MontoDescuento, Sum(detalle.Total) as Total, empresa.Nombre AS Empresa, empresa.Direccion AS DireccionEmpresa, empresa.Ciudad, CAST('' AS image) AS imageEmpresa,
                      (SELECT TOP (1) Imagen
					   ...
					   a.HoraRegistro, ISNULL(inc.Monto, 0) AS IncrementoVenta
FROM     dbo.Ventas AS a INNER JOIN
                  ...
                  dbo.Personal AS per ON per.Id = a.PersonalId LEFT JOIN
                  dbo.VentasIncrementoAplicado AS inc ON inc.VentaId = a.Id
WHERE   a.Id =@Id
...
empresa.Nombre ,empresa .Direccion,empresa .Ciudad,per.NombrePersonal,a.Glosa,a.TipoVenta,a.FechaVencimientoCredito,a.Descuento,a.HoraRegistro,inc.Monto
